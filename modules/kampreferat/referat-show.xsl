...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0" xmlns="http://www.w3.org/1999/xhtml" xmlns:portal="http://www.enonic.com/cms/xslt/portal" xmlns:util="enonic:utilities" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="content">
    ...
    <xsl:if test="contentdata/stikktittel">
      <p class="stikktittel"><xsl:value-of select="contentdata/stikktittel"/></p>
    </xsl:if>
    <span class="byline">
      <xsl:value-of select="util:format-date(@publishfrom, /result/context/@languagecode, 'long', true())"/>
    </span>
    <xsl:choose>
    ...
    <!-- denne brukes i portlets og lister
      <xsl:if test="contentdata/kortingress">
      <p class="preface">
      <xsl:value-of disable-output-escaping="yes" select="replace(contentdata/kortingress, '\n', '&lt;br /&gt;')"/>
      </p>
      </xsl:if>
    -->
    <xsl:if test="contentdata/langingress">
      <p class="preface">
        <xsl:value-of disable-output-escaping="yes" select="replace(contentdata/langingress, '\n', '&lt;br /&gt;')"/>
      </p>
    </xsl:if>
    ...
    <!-- 
      hjemmelag key="135"/>
      <bortelag key="135"/>
      <goal-hjemme>5</goal-hjemme>
      <goal-borte>0</goal-borte>
      <goal-pause>(1 - 0), (3 - 0), (5 - 0)</goal-pause>
      −
      <more>
      −
      <more-text>
    -->
    <p>Kampoppsett:
    <xsl:value-of select="/result/contents/relatedcontents/content[@key = current()/contentdata/hjemmelag/@key]/contentdata/kortnavn"/>
      <xsl:text> - </xsl:text>
      <xsl:value-of select="/result/contents/relatedcontents/content[@key = current()/contentdata/bortelag/@key]/contentdata/kortnavn"/>
    </p>
    <p>
      Mål hjemmelag: <xsl:value-of select="contentdata/goal-hjemme"/>
    </p>
    <p>
      Mål bortelag: <xsl:value-of select="contentdata/goal-borte"/>
    </p>
    <p>
      Pauseresultat: <xsl:value-of select="contentdata/goal-pause"/>
    </p>
    <p>En for-each-løkke for eventuelle fritekst editorer instanser</p>
    <xsl:if test="contentdata/more/more-text/node()">
      <xsl:for-each select="contentdata/more">
        <div class="editor">
          <xsl:call-template name="process-html-area.process-html-area">
            <xsl:with-param name="region-width" tunnel="yes" select="$region-width"/>
            <xsl:with-param name="filter" tunnel="yes" select="$config-filter"/>
            <xsl:with-param name="imagesize" tunnel="yes" select="$config-imagesize"/>
            <xsl:with-param name="document" select="more-text"/>
            <xsl:with-param name="image" tunnel="yes" select="/result/contents/relatedcontents/content"/>
          </xsl:call-template>
        </div>
      </xsl:for-each>
        ...
        <xsl:if test="/result/contents/relatedcontents/content[@key = current()/contentdata/articles/content/@key]">
          ...
          <ul>
            <xsl:for-each select="contentdata/articles/content[@key = /result/contents/relatedcontents/content/@key]">
              ...
              <li>
                <a href="{portal:createContentUrl($current-article/@key, ())}" title="{$current-article/title}">
