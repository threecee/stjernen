...
jQuery.fn.imageGallery = function(options) {
	...
	var _current = settings.start;

	var createButtons = function() {
        var buttons = '';
        for (var i=0; i<itemsLn; i++) {
            var title = $(items[i]).find('img').attr('title'); 
            buttons += '<a href="javascript:void(0);" title="'+title+'" id="foo'+i+'" class="button">'+(i+1)+'</a>';
        }
        
        $(element).find('.image-container').before('<div class="navigation-menu">'+buttons+'</div>');
        for (var i=0; i<itemsLn; i++) {
            $(element).find('.navigation-menu/.button:eq('+i+')').attr('pos',i);
            $(element).find('.navigation-menu/.button:eq('+i+')').click(function() { browse($(this).attr('pos')) })
        }
    }
    
    var createImgButtons = function() {
        var buttons = '';
        for (var i=0; i<itemsLn; i++) {
            var imgEl = $(items[i]).find('img');
            var title = imgEl.attr('title'); 
            var img = imgEl.attr('src'); 
            //buttons += '<a href="javascript:void(0);" title="'+title+'" id="foo'+i+'" class="button" style="background-image:url('+img+')"></a>';
            buttons +='<a href="javascript:void(0);" title="'+title+'" id="foo'+i+'" class="button"><img src="'+img+'" alt="'+title+'" style="width:17px;heigth:10px;border:0"></a>';
            
        }
        $(element).find('.image-container').after('<div class="navigation-menu">'+buttons+'</div>');
        for (var i=0; i<itemsLn; i++) {
            $(element).find('.navigation-menu/.button:eq('+i+')').attr('pos',i);
            $(element).find('.navigation-menu/.button:eq('+i+')').click(function() { browse($(this).attr('pos')) })
        }
    ...
    var nextImage = function(args) {
        $(items).hide(); 
        if (args.last !=null) {
            $(items[args.last]).show().css('zIndex', '0'); 
        }
        $(items[args.current]).css('zIndex', '1'); 
        if (settings.transition == 'fadeIn') {
            $(items[args.current]).fadeIn('slow'); 
        }
        else if(settings.transition == 'show') {
            $(items[args.current]).show('slow'); 
        }
        else if(settings.transition == 'slideDown') {
            $(items[args.current]).slideDown('slow'); 
        }
    }
    
    var browse = function(current) {
        var last = '';
        for (var i=0; i<itemsLn; i++) {
            if ($(items[i]).css('z-index') == 1) {
               last = i;
            }
        }
        if (current !=last) {
    ...
    var slideShow = function() {
        ...
        nextImage({
            current:_current, last:last
        });
    }

	var element = this;
	var items = $(element).find('.image-container/.image');
	var itemsLn = $(items).size();

    $(items).css({
        position:"absolute",top:"0",left:"0"
    });

    $(items).find('.image-description').addClass('abs');
    
    if (settings.type == 'gallery') {
        createButtons();
    }
    
    if (settings.type == 'galleryImg') {
        createImgButtons();
    }

    if (settings.type == 'slideshow') {
        var interval = setInterval(slideShow,settings.interval*1000);
    }
	
    nextImage({current:settings.start, last:null});
